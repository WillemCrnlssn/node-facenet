...
class FaceGrouper {
    ...
    createExplorerData(workdir) {
        config_1.log.verbose('FaceGrouper', 'createExplorerData(%s)', workdir ? workdir : '');
        if (!workdir) {
        ...
        const explorer = {
            ...
            getPath: (self) => {
                config_1.log.silly('FaceGrouper', 'createExplorerData() getPath(%s)', self.name);
            ...
            children: (self) => {
                ...
                config_1.log.silly('FaceGrouper', 'createExplorerData() children(%s)', self.name);
                ...
                try {
                    ...
                    for (const child of children) {
                        ...
                        config_1.log.silly('FaceGrouper', 'createExplorerData() children() for(child:%s)', completePath);
                        const resultChild = {
                            name: child,
                    ...
                    config_1.log.error('FaceGrouper', 'createExplorerData() exception: %s', e);
                    // fail safe
                }
                return result;
            },
    ...
    bindSelectAction(tree) {
        ...
        tree.on('select', (node) => __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                config_1.log.error('FaceGrouper', 'bindSelectAction() tree on select exception: %s', e);
            }
        }));
    }
    process(file) {
        return __awaiter(this, void 0, void 0, function* () {
            config_1.log.verbose('FaceGrouper', 'process(%s)', file);
            this.frame.emit('image', file);
            const faceList = yield this.alignmentCache.align(file);
            config_1.log.silly('FaceGrouper', 'process() faceList.length:%d', faceList.length);
            for (const face of faceList) {
                try {
                    ...
                    config_1.log.silly('FaceGrouper', 'process() face:%s embedding:%s', face, face.embedding);
                }
                catch (e) {
                    config_1.log.error('FaceGrouper', 'process() exception:%s', e);
                }
            }
        });
    }
}
exports.FaceGrouper = FaceGrouper;
//# sourceMappingURL=face-grouper.js.map